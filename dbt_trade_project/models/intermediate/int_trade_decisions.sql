...
current_state as (
    {% if adapter.get_relation(database=target.database, schema='CURATED', identifier='CURRENT_TRADES') %}
    select trade_id, version as existing_version
    from {{ target.database }}.CURATED.CURRENT_TRADES
    {% else %}
